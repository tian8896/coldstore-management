...
var currentSupplierName = null; // 供应商自己的名称（用于在采购记录中识别）
...
function generateSeq(cb, dateOverride) {
  ...
  if (!dbRef) { cb && cb(''); return; }
  if (!seqCounterRef) {
    seqCounterRef = dbRef.parent.child('csm_seq_counter');
  }
  var useDate = dateOverride || new Date();
  var y = useDate.getFullYear();
  var m = ('0' + (useDate.getMonth() + 1)).slice(-2);
  var d = ('0' + (useDate.getDate())).slice(-2);
  var dateStr = '' + y + m + d; // e.g. "20260410"

  seqCounterRef.once('value').then(function(snap) {
    var data = snap.val() || {};
    var cnt = (data[dateStr] || 0) + 1;
    seqCounterRef.child(dateStr).set(cnt).then(function() {
      var seq = dateStr + ('00' + cnt).slice(-3);
      cb && cb(seq);
    });
  }).catch(function() {
    cb && cb(dateStr + '001');
...
function backfillSeq() {
  if (!dbRef || !seqCounterRef) return;
  var recsWithoutSeq = recs.filter(function(r) { return !r.seq && !r.type; });
  if (recsWithoutSeq.length === 0) return;
  console.log('Backfilling seq for', recsWithoutSeq.length, 'records');
  ...
  // 按日期分组统计
  var byDate = {};
  recsWithoutSeq.forEach(function(r) {
  ...
  // 获取当前计数器
  seqCounterRef.once('value').then(function(snap) {
    var counters = Object.assign({}, snap.val() || {});

    var batch = {};
    var pending = recsWithoutSeq.length;
    ...
    recsWithoutSeq.forEach(function(r) {
      ...
      var seq = key + ('00' + counters[key]).slice(-3);
      batch['csm_seq_counter/' + key] = counters[key];
      batch['csm_records_v3/' + r.id + '/seq'] = seq;
    });

    // 批量写入
    dbRef.parent.update(batch).then(function() {
...
function backfillCheckoutSeq() {
  if (!dbRef || !seqCounterRef) return;
  var recsWithoutSeq = recs.filter(function(r) { return r.type === 'checkout' && !r.seq; });
  if (recsWithoutSeq.length === 0) return;
  console.log('Backfilling checkout seq for', recsWithoutSeq.length, 'records');

  recsWithoutSeq.sort(function(a, b) { return new Date(a.dep || 0) - new Date(b.dep || 0); });
  ...
  seqCounterRef.once('value').then(function(snap) {
    ...
    recsWithoutSeq.forEach(function(r) {
      var d = r.dep ? new Date(r.dep) : new Date();
      var y = d.getFullYear();
      var m = ('0' + (d.getMonth() + 1)).slice(-2);
      var dd = ('0' + (d.getDate())).slice(-2);
      var key = '' + y + m + dd;
    ...
    dbRef.parent.update(batch).then(function() {
      console.log('Checkout backfill complete');
    }).catch(function(e) {
      console.error('Checkout backfill error:', e);
    });
  });
}

// 为已有采购记录补充 seq（根据采购日期生成）
function backfillPurchaseSeq() {
  if (!purchaseRef || !seqCounterRef) return;
  var recsWithoutSeq = purchaseRecs.filter(function(r) { return !r.seq; });
  if (recsWithoutSeq.length === 0) return;
  console.log('Backfilling purchase seq for', recsWithoutSeq.length, 'records');

  recsWithoutSeq.sort(function(a, b) { return new Date(a.purchaseDate || 0) - new Date(b.purchaseDate || 0); });
  ...
  seqCounterRef.once('value').then(function(snap) {
    ...
    recsWithoutSeq.forEach(function(r) {
      var d = r.purchaseDate ? new Date(r.purchaseDate + 'T00:00:00') : new Date();
      var y = d.getFullYear();
      var m = ('0' + (d.getMonth() + 1)).slice(-2);
      var dd = ('0' + (d.getDate())).slice(-2);
      var key = '' + y + m + dd;
      ...
      var seq = key + ('00' + counters[key]).slice(-3);
      batch['csm_seq_counter/' + key] = counters[key];
      batch['csm_purchase/' + r.id + '/seq'] = seq;
    });

    dbRef.parent.update(batch).then(function() {
      console.log('Purchase backfill complete');
    }).catch(function(e) {
      console.error('Purchase backfill error:', e);
    });
  });
}
...
function initApp() {
  ...
  try {
    ...
    supplierRef = firebase.database().ref('csm_supplier_recs');
    ...
    auth.onAuthStateChanged(function(user) {
      if (user) {
        ...
        // ✅ 立即隐藏登录弹窗
        var loginModal = gid('loginModal');
        if (loginModal) {
          loginModal.style.display = 'none';
          loginModal.classList.remove('sh');
        }

        // 获取用户角色（异步，精确更新显示）
        var usersRef = firebase.database().ref('csm_users/' + user.uid);
        usersRef.once('value').then(function(snap) {
          var userData = snap.val();
          var userDisplay = gid('currentUserDisplay');
          if (userData) {
            isAdmin = userData.role === 'admin';
            isLogistics = userData.role === 'logistics';
            isSupplier = userData.role === 'supplier';
            currentSupplierName = userData.supplierName || null;
            // 精确更新角色显示
            var newRoleText = isLogistics ? '物流公司' : (isSupplier ? '供应商' : '管理员');
            if (userDisplay) {
              userDisplay.textContent = (user.email || 'User') + ' (' + newRoleText + ')';
            }
            if (isLogistics) {
              showLogisticsView();
            } else if (isSupplier) {
            ...
            isSupplier = false;
            if (userDisplay) {
              userDisplay.textContent = (user.email || 'User') + ' (管理员)';
            }
            showAdminView();
            firebase.database().ref('csm_users/' + user.uid).set({
              email: user.email,
              role: 'admin',
              createdAt: firebase.database.ServerValue.TIMESTAMP
            });
          }
          toast('✅ 欢迎 ' + (user.email || 'User'), 'ok');
        });
      } else {
        // 用户未登录，显示登录弹窗
        console.log('Firebase Auth: User not logged in');
        currentUser = null;
    ...
    // 监听供应商采购数据
    supplierRef.on('value', function(snap) {
      supplierRecs = [];
      var data = snap.val() || {};
      ...
      if (hasMigration) {
        supplierRecs.forEach(function(r) {
          if (!r.status || r.status === 'draft') {
            supplierRef.child(r.id).update({ status: 'draft' });
          }
        });
      }
...
// Firebase Auth 登录处理
function doLogin() {
  var email = (gid('login-username').value || '').trim();
  var password = (gid('login-password').value || '').trim();

  if (!email || !password) {
    gid('login-error').textContent = '请输入邮箱和密码';
  ...
  // 使用 Firebase Auth 登录
  if (!auth) {
    gid('login-error').textContent = 'Firebase 未初始化，请刷新页面重试';
    gid('login-error').style.display = 'block';
    console.error('auth is null - Firebase Auth not initialized');
    return;
  }
  auth.signInWithEmailAndPassword(email, password)
    .then(function(userCredential) {
      // 登录成功 - onAuthStateChanged 会处理后续逻辑
    ...
    .catch(function(error) {
      ...
      if (error.code === 'auth/user-not-found') {
        ...
        errorMsg = 'Firebase 配置错误，请联系管理员';
      }
      // 显示详细错误原因
      gid('login-error').textContent = errorMsg + ' (' + (error.code || 'unknown') + ')';
      gid('login-error').style.display = 'block';
    });
}
...
function doRegister() {
  ...
  if (password.length < 6) {
    gid('login-error').textContent = '密码至少6位';
    gid('login-error').style.display = 'block';
    return;
  }
  ...
  auth.createUserWithEmailAndPassword(email, password)
    .then(function(userCredential) {
      console.log('Firebase Auth: Register success', userCredential.user.email);

      // 创建用户角色记录 - 新用户默认是物流公司
      var userId = userCredential.user.uid;
      firebase.database().ref('csm_users/' + userId).set({
        email: email,
        role: 'logistics', // 默认物流公司角色
        createdAt: firebase.database.ServerValue.TIMESTAMP
      });
    ...
    .catch(function(error) {
      ...
      gid('login-error').textContent = errorMsg;
      gid('login-error').style.display = 'block';
    });
}
...
function doGoogleLogin() {
  ...
  auth.signInWithPopup(provider)
    .then(function(result) {
      console.log('Firebase Auth: Google login success', result.user.email);

      // 保存或更新用户信息到数据库
      var userId = result.user.uid;
      firebase.database().ref('csm_users/' + userId).update({
...
function showAdminView() {
  ...
  gid('supplierView').style.display = 'none';
  document.querySelectorAll('.right')[2].style.display = 'block';
...
function showLogisticsView() {
  gid('supplierView').style.display = 'none';
  document.querySelectorAll('.right')[2].style.display = 'none';
  ...
  gid('logisticsUserName').textContent = currentUserEmail || currentUser;
  updateSettingsButton();
...
function showSupplierView() {
  gid('logisticsView').style.display = 'none';
  // 隐藏 admin 主界面（第三个 .right）
  var rights = document.querySelectorAll('.right');
  if (rights[2]) rights[2].style.display = 'none';
  // 隐藏右侧面板
  if (rights[1]) rights[1].style.display = 'none';
  gid('supplierView').style.display = 'block';
  gid('userInfo').style.display = 'none';
  document.querySelector('header h1').textContent = '🏭 供应商采购系统 / Supplier Portal';
  gid('supplierUserName').textContent = currentUserEmail || currentUser;
  updateSettingsButton();
  renderSupplierTable();
}
...
function clearSupplierSearch() {
  if (gid('supplier-search-date-start')) gid('supplier-search-date-start').value = '';
  if (gid('supplier-search-date-end')) gid('supplier-search-date-end').value = '';
...
function onUserRoleChange() {
  var role = gid('new-user-role') ? gid('new-user-role').value : 'admin';
  var nameRow = gid('supplier-name-row');
  if (nameRow) {
    nameRow.style.display = (role === 'supplier') ? 'flex' : 'none';
  }
}
...
function openSupplierForm() {
  ...
  gid('supplier-product').value = '';
  gid('supplier-qty').value = '';
...
function saveSupplierRec() {
  ...
  var product = (gid('supplier-product').value || '').trim();
  var qty = parseInt(gid('supplier-qty').value) || 0;
  ...
  if (!product) { toast('请输入品名 / Enter product name', 'err'); return; }
  ...
  if (id) {
    ...
    supplierRef.child(id).set(rec).then(function() {
...
function undoSubmitSupplierRec(id) {
  ...
  supplierRef.child(id).update({
    status: 'draft',
    submittedBy: firebase.database.ServerValue.delete,
    submittedAt: firebase.database.ServerValue.delete
  }).then(function() {
...
function confirmSupplierRec(id) {
  var rec = supplierRecs.find(function(r) { return r.id === id; });
  if (!rec) return;
  if (rec.status !== 'submitted') { toast('⚠️ 只有已提交状态才能确认', 'err'); return; }
  if (!confirm('⚠️ 确认后记录将正式锁定，不可撤销！\n确认这条采购记录？')) return;
  supplierRef.child(id).update({
    status: 'confirmed',
    confirmedBy: currentUserEmail,
    confirmedAt: new Date().toISOString()
  }).then(function() {
    toast('✅ 已确认，采购记录正式锁定 / Confirmed, record permanently locked', 'ok');
  });
}
...
function editSupplierRec(id) {
  ...
  gid('supplier-product').value = rec.product || '';
  gid('supplier-qty').value = rec.qty || '';
...
function renderSupplierTable() {
  ...
  // 按日期倒序
  filtered.sort(function(a, b) { return new Date(b.purchaseDate) - new Date(a.purchaseDate); });
  ...
  var html = filtered.map(function(r) {
    ...
    if (isSupplier) {
      if (status === 'draft') {
        ...
        actionBtns =
          '<button class="abtn" style="background:#ffb300;color:#fff" onclick="undoSubmitSupplierRec(\'' + r.id + '\')">↩ 撤销</button>';
      } else {
    ...
    return '<tr>' +
      ...
      '<td><a href="javascript:void(0)" onclick="' + cnClickFn + '" style="color:#00bfff;font-weight:bold;text-decoration:underline">' + (r.cn || '-') + '</a></td>' +
      '<td>' + (r.supplier || '-') + '</td>' +
      '<td>' + (r.product || '-') + '</td>' +
...
function openSupplierCNDetail(id) {
  ...
  if (purchaseRecs && purchaseRecs.length) {
    linked = purchaseRecs.find(function(p) { return p.cn === rec.cn; });
  }
  ...
  var html = '<div style="text-align:left;min-width:280px">';
  ...
  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">品名</td><td style="padding:6px">' + (rec.product || '-') + '</td></tr>';
  ...
  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">数量</td><td style="padding:6px">' + (rec.qty || 0) + '</td></tr>';
  ...
  if (linked) {
    ...
    html += '<div style="color:#00aa00;font-weight:bold;margin-bottom:6px">✅ Warehouse1 已入库 / Already in Warehouse</div>';
    html += '<table style="width:100%;border-collapse:collapse;font-size:13px">';
    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">入库日期</td><td style="padding:4px">' + (linked.purchaseDate || '-') + '</td></tr>';
    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">停柜费</td><td style="padding:4px">' + ((linked.demurrage || 0) + ' AED') + '</td></tr>';
    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">清关费</td><td style="padding:4px">' + ((linked.customs || 0) + ' AED') + '</td></tr>';
    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">冷库费</td><td style="padding:4px">' + ((linked.coldfee || 0) + ' AED') + '</td></tr>';
    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">认证费</td><td style="padding:4px">' + ((linked.attestation || 0) + ' AED') + '</td></tr>';
    html += '<tr><td style="padding:4px;color:#666">翻包费</td><td style="padding:4px">' + ((linked.repack || 0) + ' AED') + '</td></tr>';
    html += '</table></div>';
  } else {
    html += '<div style="margin-top:12px;padding:10px;background:#fff8e1;border-radius:6px;border:1px solid #ffb300">';
    html += '<div style="color:#ff8f00;font-weight:bold">⚠️ 尚未在 Warehouse1 入库 / Not yet in Warehouse</div>';
    html += '<div style="margin-top:4px;font-size:13px;color:#666">请等待 Warehouse 确认入库后，在此显示详细信息。</div></div>';
  }
...
function checkOut() {
  ...
  console.log('checkOut values:', {cn: cn, pallets_out: pallets_out, items_out: items_out, dt: dt});

  if (!cn || cn.length < 2) { toast('请输入集装箱号码', 'err'); console.log('validation failed: cn'); return; }
  if (!dt) { toast('请输入出库日期', 'err'); console.log('validation failed: dt'); return; }

  // 找到入库记录
  console.log('Searching for:', cn, 'in store:', currentColdStore);
  console.log('Available records:', recs.map(function(r){return {cn:r.cn, store:r.store, dep:r.dep, type:r.type}}));

  var rec = recs.find(function(r) {
    return r.cn === cn && !r.dep && !r.type && r.store === currentColdStore;
  });
  console.log('Found record:', rec);
  if (!rec) {
    toast('未找到在库记录: ' + cn + ' (冷库' + currentColdStore + ')', 'err');
    console.log('rec not found, recs:', recs.filter(function(r){return r.cn === cn}));
    return;
  }
  ...
  Object.keys(cnGroups).sort().forEach(function(cn) {
    ...
    while (firstLoop || (currentDate <= endDate && outRecs.length > 0)) {
      ...
      html.push('<tr style="' + rowBg + '">' +
        '<td style="font-size:13px;color:#0066cc">' + (inRec.seq || '-') + '</td>' +
      ...
      weekOutRecs.forEach(function(or) {
        ...
        html.push('<tr style="background:#f0fff0">' +
          '<td style="padding-left:20px;color:#0066cc;font-size:13px">' + (or.seq || '-') + '</td>' +
...
// 供应商采购记录
var SUPPLIER_RECS_KEY = 'csm_supplier_recs_v1';
var supplierRecs = [];
var supplierRef = null;
...
function addPurchase() {
  ...
  rows.forEach(function(row, idx) {
    ...
    generateSeq(function(seq) {
      rec.seq = seq;
      if (purchaseRef) {
        purchaseRef.child(id).set(rec).then(function(){console.log('Saved:',id,'seq:',seq);}).catch(function(e){console.error('Error:',e);});
      }
...
function renderPurchase() { console.log("purchaseRecs:", purchaseRecs);
  ...
  var warehouseCNs = {};
  purchaseRecs.forEach(function(r) { if (r.cn) warehouseCNs[r.cn] = true; });

  var supplierOnlyRecs = supplierRecs.filter(function(r) {
    if (!r.cn) return false;
    if (warehouseCNs[r.cn]) return false; // 已在 warehouse1 中，不再重复显示
    var matchDate = !searchDate || (r.purchaseDate || '').indexOf(searchDate) !== -1;
    var matchCn = !searchCn || (r.cn || '').indexOf(searchCn) !== -1;
    var matchSupplier = !searchSupplier || (r.supplier || '').toUpperCase().indexOf(searchSupplier) !== -1;
    return matchDate && matchCn && matchSupplier;
  ...
  if (filteredRecs.length === 0 && supplierOnlyRecs.length === 0) { tb.innerHTML = ''; es.style.display = 'block'; return; }
  es.style.display = 'none';
  ...
  Object.keys(cnGroups).sort().forEach(function(cn) {
    ...
    for (var i = 0; i < recs.length; i++) {
      if (recs[i].cn === cn && !recs[i].type && recs[i].store === currentColdStore) {
        ...
        if (actualFee > 0) {
          coldFeeDisplay = '<strong style="color:#ff9900;background:#fff8e1;padding:2px 6px;border-radius:3px;font-size:14px">' + actualFee.toFixed(2) + '</strong>';
        } else {
          coldFeeDisplay = '<span style="color:#666;background:#fff8e1;padding:2px 6px;border-radius:3px;font-size:14px">0.00</span>';
        }
        break;
      }
    }
    ...
    var firstSeq = firstItem.seq || '-';

    html += '<tr style="background:#f9f9f9;font-weight:bold" id="pur-main-' + groupId + '">' +
      '<td style="font-size:13px;color:#0066cc">' + firstSeq + '</td>' +
      ...
      '<td style="font-family:Arial;text-transform:capitalize">'+(firstItem.supplier||'-')+'</td><td style="font-family:Arial">'+firstProduct+'</td><td>'+purchaseDate+'</td><td style="font-family:Arial">'+firstQty+'</td><td style="font-family:Arial">'+(firstItem.demurrage||0)+'</td><td style="font-family:Arial">'+(firstItem.customs||0)+'</td><td style="font-family:Arial">'+(coldFeeDisplay||'-')+'</td><td style="font-family:Arial">'+(firstItem.attestation||0)+'</td><td style="font-family:Arial">'+(firstItem.repack||0)+'</td><td style="font-family:Arial">'+(firstItem.waste||0)+'</td><td style="font-family:Arial">'+(firstItem.other||0)+'</td>' +
      '<td><strong style="color:#0066cc">'+totalAmount.toFixed(2)+'</strong></td>' +
      '<td><button type="button" class="abtn" onclick="openEditPurchase(\''+firstItem.id+'\')">✏️</button><button type="button" class="abtn x" onclick="delPurchaseGroup(\'' + cn + '\')">🗑</button></td></tr>';
    ...
    items.forEach(function(r) {
      ...
      html += '<tr class="purchase-sub-row ' + groupId + '" style="display:none;background:#fff">' +
        '<td style="font-size:13px;color:#0066cc">' + (r.seq || '-') + '</td>' +
  ...
  if (supplierOnlyRecs.length > 0) {
    // 按采购日期排序
    supplierOnlyRecs.sort(function(a, b) { return new Date(b.purchaseDate) - new Date(a.purchaseDate); });

    supplierOnlyRecs.forEach(function(r) {
      var supplierLabel = '<span style="background:#fff3e0;color:#e65100;font-size:10px;padding:1px 4px;border-radius:2px;margin-left:4px">供应商</span>';
      var cnClickFn = "openSupplierCNDetailByCN('" + r.cn + "')";

      html += '<tr style="background:#fffbf5">' +
        '<td style="font-size:13px;color:#ff9900">' + (r.seq || '-') + '</td>' +
        '<td><a href="javascript:void(0)" onclick="' + cnClickFn + '" style="color:#ff9900;font-weight:bold;text-decoration:underline">' + (r.cn || '-') + '</a>' + supplierLabel + '</td>' +
        '<td style="font-family:Arial;text-transform:capitalize;color:#666">' + (r.supplier || '-') + '</td>' +
        '<td style="font-family:Arial;text-transform:capitalize">' + (r.product || '-') + '</td>' +
        '<td>' + (r.purchaseDate ? fdt(r.purchaseDate+'T00:00:00') : '-') + '</td>' +
        '<td style="font-family:Arial">' + (r.qty || 0) + '</td>' +
        '<td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td>' +
        '<td style="color:#999">待入库</td>' +
        '<td><button type="button" class="abtn" onclick="editSupplierRecByCN(\'' + r.cn + '\')">✏️</button></td>' +
      '</tr>';
    });
  }
...
// 按集装箱号查找供应商记录并弹出详情
function openSupplierCNDetailByCN(cn) {
  var rec = supplierRecs.find(function(r) { return r.cn === cn; });
  if (!rec) return;
  openSupplierCNDetail(rec.id);
}

// 按集装箱号编辑供应商记录
function editSupplierRecByCN(cn) {
  var rec = supplierRecs.find(function(r) { return r.cn === cn; });
  if (!rec) return;
  editSupplierRec(rec.id);
}
...
function renderUserList(users) {
  ...
  users.forEach(function(user) {
    var roleText = user.role === 'admin' ? '管理员' : '物流公司';
    var roleClass = user.role === 'admin' ? 'bdg-a' : 'bdg-d';
...
function createUser() {
  ...
  // 验证输入
  if (!email) {
    if (errorEl) { errorEl.textContent = '请输入邮箱'; errorEl.style.display = 'block'; }
    return;
  }
  if (!password || password.length < 6) {
    if (errorEl) { errorEl.textContent = '密码至少6位'; errorEl.style.display = 'block'; }
    return;
  }
  if (role === 'supplier' && !supplierName) {
    if (errorEl) { errorEl.textContent = '供应商名称不能为空'; errorEl.style.display = 'block'; }
    return;
  }

  var roleLabel = role === 'admin' ? '管理员' : (role === 'logistics' ? '物流公司' : '供应商');

  // 使用 Firebase Auth 创建账号
  auth.createUserWithEmailAndPassword(email, password)
    .then(function(userCredential) {
...
function showSuggest(inputEl, type) {
  ...
  console.log('showSuggest:', type, 'val:', val, 'list:', list, 'suggestEl:', !!suggestEl);
...
function openLogisticsAddForm(id) {
  ...
  gid('logistics-date').value = yyyy + '-' + mm + '-' + dd;
  gid('logistics-time').value = hh + ':' + mi;
  gid('logistics-fee').value = '0';
  gid('logistics-discount').value = '0';
  gid('logistics-remark').value = '';
  gid('logistics-payment-status').value = 'unpaid';
...
function saveLogisticsFee() {
  ...
  var addTime = (gid('logistics-time').value || '').trim();
  ...
  var paymentStatus = gid('logistics-payment-status').value || 'unpaid';
  ...
  if (id) {
    ...
    if (idx !== -1) {
      ...
      fees[idx].addTime = addTime || fees[idx].addTime || '';
      fees[idx].fee = fee;
      ...
      fees[idx].paymentStatus = paymentStatus;
    }
    ...
    fees.push({
      ...
      remark: remark,
      addedBy: currentUser,
      addTimeISO: new Date().toISOString(),
      confirmed: false,
      confirmedBy: null,
      confirmTime: null,
...
function clearLogisticsSearch() {
  gid('logistics-search-date-start').value = '';
  gid('logistics-search-date-end').value = '';
  gid('logistics-search-cn').value = '';
  renderLogisticsTable();
}
...
function renderLogisticsTable() {
  ...
  var fees = getLogisticsFees();
  console.log('[Logistics] fees count:', fees.length, 'fees:', fees);

  var searchCN = (gid('logistics-search-cn').value || '').trim().toUpperCase();
  var searchDateStart = (gid('logistics-search-date-start').value || '').trim();
  var searchDateEnd = (gid('logistics-search-date-end').value || '').trim();
  var fees = getLogisticsFees();

  if (searchCN || searchDateStart || searchDateEnd) {
    fees = fees.filter(function(f) {
      var matchCN = !searchCN || (f.cn || '').toUpperCase().indexOf(searchCN) !== -1;
      var fDate = f.addDate || '';
      var matchDate = true;
      if (searchDateStart && fDate < searchDateStart) matchDate = false;
      if (searchDateEnd && fDate > searchDateEnd) matchDate = false;
      return matchCN && matchDate;
    });
  }
  ...
  var html = fees.map(function(f) {
    ...
    var purchaseMatch = '';
    if (typeof purchaseRecs !== 'undefined' && purchaseRecs && purchaseRecs.length) {
    ...
    var paymentHTML = '';
    var ps = f.paymentStatus || 'unpaid';
    if (ps === 'paid') {
      paymentHTML = '<span style="color:#00aa00;font-weight:bold;background:#e8f5e9;padding:2px 8px;border-radius:4px">✅ PAID</span>';
    } else if (ps === 'partial') {
      paymentHTML = '<span style="color:#ff9900;background:#fff8e1;padding:2px 8px;border-radius:4px">⏳ PARTIAL</span>';
    } else {
      paymentHTML = '<span style="color:#cc0000;background:#fce4ec;padding:2px 8px;border-radius:4px">❌ UNPAID</span>';
    }

    return '<tr style="' + statusClass + '">' +
      '<td><strong>' + (f.cn || '-') + '</strong>' + purchaseMatch + '</td>' +
      '<td style="white-space:nowrap">' + (f.addDate || '-') + (f.addTime ? '<br><span style="font-size:11px;color:#888">' + f.addTime + '</span>' : '') + '</td>' +
  ...
  tb.innerHTML = html + '<tr style="background:#e8f5e9;font-weight:bold"><td colspan="2">📊 Total</td><td>' + totalFee.toFixed(2) + ' AED</td><td style="color:#ff4444">-' + totalDiscount.toFixed(2) + ' AED</td><td></td><td></td><td></td></tr>';
}
