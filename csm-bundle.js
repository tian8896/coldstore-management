...
// 退出登录
function doLogout() {  setSupplierPortalLayout(false);  auth.signOut()    .then(function() {      console.log('Firebase Auth: Logged out');      currentUser = null;      currentUserEmail = null;      isAdmin = false;      isStaff = false;      isLogistics = false;      isSupplier = false;      currentSupplierName = null;      hasCloudSettingsSnapshot = false;      currentMainSuite = 'w1';      try { window.isAdmin = false; window.isStaff = false; } catch (eW3) {}      try { sessionStorage.removeItem('csm_main_suite'); } catch (eLo) {}      
// 重置界面      
...
function renderSalesDashCards() {
  ...
  var unpaid = conf.filter(function(o) {
    var p = String(o.paymentStatus || '');
    return p === 'credit' || p === 'cash_pending';
  });
...
function renderSalesFinanceTable() {
  ...
  var sumUn = conf.filter(function(o) {
    var p = String(o.paymentStatus || '');
    return p === 'credit' || p === 'cash_pending';
  })
