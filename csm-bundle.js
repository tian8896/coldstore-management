...
var warehouseCNs = {};  purchaseRecs.forEach(function(r) { if (r.cn) warehouseCNs[r.cn] = true; });  var supplierOnlyRecs = supplierRecs.filter(function(r) {    if (!r.cn) return false;    if (warehouseCNs[r.cn]) return false; 
...
// 按采购日期排序    
supplierOnlyRecs.sort(function(a, b) { return new Date(b.purchaseDate) - new Date(a.purchaseDate); });    supplierOnlyRecs.forEach(function(r) {      var supplierLabel = '<span style="background:#fff3e0;color:#e65100;font-size:10px;padding:1px 4px;border-radius:2px;margin-left:4px">供应商</span>';      var cnClickFn = "openSupplierCNDetailByCN('" + r.cn + "')";      html += '<tr style="background:#fffbf5">' +        '<td style="font-size:13px;color:#ff9900">' + (r.seq || '-') + '</td>' +        '<td><a href="javascript:void(0)" onclick="' + cnClickFn + '" style="color:#ff9900;font-weight:bold;text-decoration:underline">' + (r.cn || '-') + '</a>' + supplierLabel + '</td>' +        '<td style="font-family:Arial;text-transform:capitalize;color:#666">' + (r.supplier || '-') + '</td>' +        '<td style="font-family:Arial;text-transform:capitalize">' + (r.product || '-') + '</td>' +        '<td>' + (r.purchaseDate ? fdt(r.purchaseDate+'T00:00:00') : '-') + '</td>' +        '<td style="font-family:Arial">' + (r.qty || 0) + '</td>' +        '<td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td>' +        '<td style="color:#999">待入库</td>' +        '<td><button type="button" class="abtn" onclick="editSupplierRecByCN(\'' + r.cn + '\')">✏️</button></td>' +      '</tr>';    });  }  tb.innerHTML = html;}
// 按集装箱号查找供应商记录并弹出详情
