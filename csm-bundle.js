...
// 编辑（保留原状态）    
rec.id = id;    rec.status = existing ? existing.status : 'draft';    rec.ownerUid = (existing && existing.ownerUid) ? existing.ownerUid : currentUser;    rec.updatedBy = currentUserEmail;    rec.updateTime = new Date().toISOString();    supplierRef.child(id).set(rec).then(function() {      toast('✅ 记录已更新 / Updated', 'ok');      clSupplierModal();    });  } else {    
// 新增，先生成序列号    
