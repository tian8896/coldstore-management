...
// 显示管理员视图
function showAdminView() {  var lv = gid('logisticsView');  var sv = gid('supplierView');  if (lv) lv.style.display = 'none';  if (sv) sv.style.display = 'none';  var rights = document.querySelectorAll('.right');  if (rights[2]) rights[2].style.display = 'block';  var ui = gid('userInfo');  if (ui) ui.style.display = 'flex';  updateSettingsButton();  renderPurchase();}
// 显示清关公司视图
function showLogisticsView() {  var sv = gid('supplierView');  if (sv) sv.style.display = 'none';  var rights = document.querySelectorAll('.right');  if (rights[2]) rights[2].style.display = 'none';  var lv2 = gid('logisticsView');  if (lv2) lv2.style.display = 'block';  var ui2 = gid('userInfo');  if (ui2) ui2.style.display = 'none';  var h1a = document.querySelector('header h1');  if (h1a) h1a.textContent = '物流公司系统 / Logistics System';  var lname = gid('logisticsUserName');  if (lname) lname.textContent = currentUserEmail || currentUser;  updateSettingsButton();  renderLogisticsTable();}
// 显示供应商视图
function showSupplierView() {  var lv3 = gid('logisticsView');  if (lv3) lv3.style.display = 'none';  var rights2 = document.querySelectorAll('.right');  if (rights2[2]) rights2[2].style.display = 'none';  var sv2 = gid('supplierView');  if (sv2) sv2.style.display = 'block';  var ui3 = gid('userInfo');  if (ui3) ui3.style.display = 'none';  var h1b = document.querySelector('header h1');  if (h1b) h1b.textContent = '🏭 供应商采购系统 / Supplier Portal';  var sname = gid('supplierUserName');  if (sname) sname.textContent = currentUserEmail || currentUser;  updateSettingsButton();  renderSupplierTable();}
// 清空供应商搜索
...
// ============================================================
var supplierFormRowCounter = 0;function openSupplierForm() {  var now = new Date();  var y = now.getFullYear();  var mm = String(now.getMonth() + 1).padStart(2, '0');  var dd = String(now.getDate()).padStart(2, '0');  var hh = String(now.getHours()).padStart(2, '0');  var mi = String(now.getMinutes()).padStart(2, '0');  gid('supplier-id').value = '';  gid('supplier-cn').value = '';  gid('supplier-date').value = y + '-' + mm + '-' + dd;  gid('supplier-time').value = hh + ':' + mi;  gid('supplier-supplier').value = currentSupplierName || '';  gid('supplier-product').value = '';  gid('supplier-qty').value = '';  gid('supplier-shipname').value = '';  gid('supplier-bl').value = '';  gid('supplier-etd').value = '';  gid('supplier-eta').value = '';  gid('supplier-modal-title').textContent = '📦 添加采购记录 / Add Purchase Record';  gid('supplierModal').classList.add('sh');}function clSupplierModal() {  gid('supplierModal').classList.remove('sh');}function saveSupplierRec() {  var id = gid('supplier-id').value;  var cn = (gid('supplier-cn').value || '').trim().toUpperCase();  var purchaseDate = (gid('supplier-date').value || '').trim();  var purchaseTime = (gid('supplier-time').value || '').trim();  var supplier = (gid('supplier-supplier').value || '').trim();  var product = (gid('supplier-product').value || '').trim();  var qty = parseInt(gid('supplier-qty').value) || 0;  var shipname = (gid('supplier-shipname').value || '').trim();  var bl = (gid('supplier-bl').value || '').trim();  var etd = (gid('supplier-etd').value || '').trim();  var eta = (gid('supplier-eta').value || '').trim();  if (!cn) { toast('请输入集装箱号 / Enter container no.', 'err'); return; }  if (!purchaseDate) { toast('请选择采购日期 / Select purchase date', 'err'); return; }  if (!supplier) { toast('请输入供应商名称 / Enter supplier name', 'err'); return; }  if (!product) { toast('请输入品名 / Enter product name', 'err'); return; }  var existing = id ? supplierRecs.find(function(r) { return r.id === id; }) : null;  
// 只有初始状态允许编辑  
if (existing && existing.status === 'submitted') {    toast('⚠️ 记录已提交，请先撤销提交 / Already submitted, undo submit first', 'err');    clSupplierModal();    return;  }  if (existing && existing.status === 'confirmed') {    toast('⚠️ 记录已确认，无法编辑 / Already confirmed, cannot edit', 'err');    clSupplierModal();    return;  }  var rec = {    cn: cn,    purchaseDate: purchaseDate,    purchaseTime: purchaseTime,    supplier: supplier,    product: product,    qty: qty,    shipname: shipname,    bl: bl,    etd: etd,    eta: eta,    addedBy: currentUserEmail,    addTime: new Date().toISOString(),    status: 'draft'  };  if (id) {    
// 编辑（保留原状态）    
rec.id = id;    rec.status = existing ? existing.status : 'draft';    rec.updatedBy = currentUserEmail;    rec.updateTime = new Date().toISOString();    supplierRef.child(id).set(rec).then(function() {      toast('✅ 记录已更新 / Updated', 'ok');      clSupplierModal();    });  } else {    
// 新增，先生成序列号    
generateSeq(function(seq) {      rec.seq = seq;      rec.id = 'sp_' + Date.now().toString(36);      supplierRef.child(rec.id).set(rec).then(function() {        toast('✅ 记录已添加，序列号：' + seq, 'ok');        clSupplierModal();      });    }, new Date(purchaseDate + 'T00:00:00'));  }}function delSupplierRec(id) {  var rec = supplierRecs.find(function(r) { return r.id === id; });  if (!rec) return;  if (rec.status === 'confirmed') { toast('⚠️ 已确认状态，无法删除', 'err'); return; }  if (!confirm('确认删除这条采购记录？')) return;  supplierRef.child(id).remove().then(function() {    toast('✅ 已删除 / Deleted', 'ok');  });}
// 提交记录（初始 → 已提交）
function submitSupplierRec(id) {  var rec = supplierRecs.find(function(r) { return r.id === id; });  if (!rec) return;  if (rec.status !== 'draft') { toast('⚠️ 只有初始状态才能提交', 'err'); return; }  if (!confirm('确认提交这条记录？提交后将进入"已提交"状态，等待管理员确认。')) return;  supplierRef.child(id).update({    status: 'submitted',    submittedBy: currentUserEmail,    submittedAt: new Date().toISOString()  }).then(function() {    toast('✅ 已提交，等待管理员确认 / Submitted, awaiting admin confirmation', 'ok');  });}
// 撤销提交（已提交 → 初始）
function undoSubmitSupplierRec(id) {  var rec = supplierRecs.find(function(r) { return r.id === id; });  if (!rec) return;  if (rec.status !== 'submitted') { toast('⚠️ 只有已提交状态才能撤销', 'err'); return; }  if (!confirm('确认撤销提交？记录将回到初始状态，可继续编辑。')) return;  supplierRef.child(id).update({    status: 'draft',    submittedBy: firebase.database.ServerValue.delete,    submittedAt: firebase.database.ServerValue.delete  }).then(function() {    toast('✅ 已撤销，可以继续编辑 / Undo submitted, can continue editing', 'ok');  });}
// 管理员确认（已提交 → 已确认，不可逆）
function confirmSupplierRec(id) {  var rec = supplierRecs.find(function(r) { return r.id === id; });  if (!rec) return;  if (rec.status !== 'submitted') { toast('⚠️ 只有已提交状态才能确认', 'err'); return; }  if (!confirm('⚠️ 确认后记录将正式锁定，不可撤销！\n确认这条采购记录？')) return;  supplierRef.child(id).update({    status: 'confirmed',    confirmedBy: currentUserEmail,    confirmedAt: new Date().toISOString()  }).then(function() {    toast('✅ 已确认，采购记录正式锁定 / Confirmed, record permanently locked', 'ok');  });}function editSupplierRec(id) {  var rec = supplierRecs.find(function(r) { return r.id === id; });  if (!rec) return;  if (rec.status === 'submitted') { toast('⚠️ 已提交状态，请先撤销提交', 'err'); return; }  if (rec.status === 'confirmed') { toast('⚠️ 已确认状态，无法编辑', 'err'); return; }  gid('supplier-id').value = id;  gid('supplier-cn').value = rec.cn || '';  gid('supplier-date').value = rec.purchaseDate || '';  gid('supplier-time').value = rec.purchaseTime || '';  gid('supplier-supplier').value = rec.supplier || '';  gid('supplier-product').value = rec.product || '';  gid('supplier-qty').value = rec.qty || '';  gid('supplier-shipname').value = rec.shipname || '';  gid('supplier-bl').value = rec.bl || '';  gid('supplier-etd').value = rec.etd || '';  gid('supplier-eta').value = rec.eta || '';  gid('supplier-modal-title').textContent = '✏️ 编辑采购记录 / Edit Purchase Record';  gid('supplierModal').classList.add('sh');}function filterSupplierTable() {  renderSupplierTable();}function renderSupplierTable() {  var container = gid('supplierView');  if (!container) return;  var searchDateStart = (gid('supplier-search-date-start') || {value:''}).value;  var searchDateEnd = (gid('supplier-search-date-end') || {value:''}).value;  var searchCn = (gid('supplier-search-cn') || {value:''}).value.trim().toUpperCase();  var filtered = supplierRecs.filter(function(r) {    if (searchDateStart && r.purchaseDate < searchDateStart) return false;    if (searchDateEnd && r.purchaseDate > searchDateEnd) return false;    if (searchCn && (r.cn || '').indexOf(searchCn) === -1) return false;    return true;  });  
// 按日期倒序  
...
if (purchaseRecs && purchaseRecs.length) {    linked = purchaseRecs.find(function(p) { return p.cn === rec.cn; });  }  var status = rec.status || 'draft';  var statusBadge = {    'draft': '<span style="background:#e3f2fd;color:#1565c0;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:bold">✏️ 初始</span>',    'submitted': '<span style="background:#fff8e1;color:#f57f17;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:bold">📤 已提交</span>',    'confirmed': '<span style="background:#e8f5e9;color:#2e7d32;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:bold">✅ 已确认</span>'  }[status];  var html = '<div style="text-align:left;min-width:280px">';  html += '<table style="width:100%;border-collapse:collapse;font-size:14px">';  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888;width:35%">集装箱号</td><td style="padding:6px;font-weight:bold;color:#00bfff">' + (rec.cn || '-') + '</td></tr>';  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">状态</td><td style="padding:6px">' + statusBadge + '</td></tr>';  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">供应商</td><td style="padding:6px">' + (rec.supplier || '-') + '</td></tr>';  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">品名</td><td style="padding:6px">' + (rec.product || '-') + '</td></tr>';  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">采购日期</td><td style="padding:6px">' + (rec.purchaseDate || '-') + (rec.purchaseTime ? ' ' + rec.purchaseTime : '') + '</td></tr>';  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">数量</td><td style="padding:6px">' + (rec.qty || 0) + '</td></tr>';  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">船名</td><td style="padding:6px">' + (rec.shipname || '-') + '</td></tr>';  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">提单号</td><td style="padding:6px">' + (rec.bl || '-') + '</td></tr>';  html += '<tr style="border-bottom:1px solid #eee"><td style="padding:6px;color:#888">预计开船日 ETD</td><td style="padding:6px">' + (rec.etd || '-') + '</td></tr>';  html += '<tr><td style="padding:6px;color:#888">预计到港日 ETA</td><td style="padding:6px">' + (rec.eta || '-') + '</td></tr>';  html += '</table>';  if (linked) {    html += '<div style="margin-top:12px;padding:10px;background:#e8f5e9;border-radius:6px;border:1px solid #4CAF50">';    html += '<div style="color:#00aa00;font-weight:bold;margin-bottom:6px">✅ Warehouse1 已入库 / Already in Warehouse</div>';    html += '<table style="width:100%;border-collapse:collapse;font-size:13px">';    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">入库日期</td><td style="padding:4px">' + (linked.purchaseDate || '-') + '</td></tr>';    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">停柜费</td><td style="padding:4px">' + ((linked.demurrage || 0) + ' AED') + '</td></tr>';    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">清关费</td><td style="padding:4px">' + ((linked.customs || 0) + ' AED') + '</td></tr>';    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">冷库费</td><td style="padding:4px">' + ((linked.coldfee || 0) + ' AED') + '</td></tr>';    html += '<tr style="border-bottom:1px solid #c8e6c9"><td style="padding:4px;color:#666">认证费</td><td style="padding:4px">' + ((linked.attestation || 0) + ' AED') + '</td></tr>';    html += '<tr><td style="padding:4px;color:#666">翻包费</td><td style="padding:4px">' + ((linked.repack || 0) + ' AED') + '</td></tr>';    html += '</table></div>';  } else {    html += '<div style="margin-top:12px;padding:10px;background:#fff8e1;border-radius:6px;border:1px solid #ffb300">';    html += '<div style="color:#ff8f00;font-weight:bold">⚠️ 尚未在 Warehouse1 入库 / Not yet in Warehouse</div>';    html += '<div style="margin-top:4px;font-size:13px;color:#666">请等待 Warehouse 确认入库后，在此显示详细信息。</div></div>';  }  html += '</div>';  gid('mcon').innerHTML = html;  gid('modal').classList.add('sh');}
// ============================================================
// USER MANAGEMENT
// ============================================================
var allUsers = [];
...
// 管理员才显示修改按钮        
var editOutBtn = isAdmin ? '<button class="abtn" onclick="showEditOutRecord(\'' + or.id + '\')" style="margin-left:4px">✏️</button>' : '';        html.push('<tr style="background:#f0fff0">' +          '<td style="padding-left:20px;color:#0066cc;font-size:13px">' + (or.seq || '-') + '</td>' +          '<td style="padding-left:20px;color:#999">' + cn + '</td>' +          '<td style="color:#999;font-family:Arial;text-transform:capitalize">' + (or.supplier || '-') + '</td>' +          '<td style="color:#999;font-family:Arial;text-transform:capitalize">' + or.product + '</td>' +          '<td style="color:#00aa00;font-weight:bold">' + fdt(or.dep) + '</td>' +          '<td>-</td>' +          '<td><span style="color:#cc0000;font-weight:bold">' + or.pallets_out + '</span> / <strong style="color:#00aa00">' + (outRemPallets >= 0 ? outRemPallets : '0') + '</strong></td>' +          '<td><span style="color:#cc0000;font-weight:bold">' + or.items_out + '</span></td>' +          '<td><strong style="color:#00aa00">' + (outRemItems >= 0 ? outRemItems : '0') + '</strong></td>' +          '<td>-</td>' +          '<td>-</td>' +          '<td>-</td>' +          '<td>-</td>' +          '<td>' + editOutBtn + '</td>' +          '</tr>');      });      currentDate = new Date(weekEnd);      currentDate.setDate(currentDate.getDate() + 1);      weekNum++;    }  });  tb.innerHTML = html.join('');}
// ============================================================
// 搜索和导出功能
// ============================================================
...
// TAB SWITCH
// ============================================================
function swTab(tab) {  document.querySelectorAll('.tab').forEach(function(t) { t.classList.remove('ac'); });  document.querySelectorAll('.tc').forEach(function(t) { t.classList.remove('ac'); });  var tabNames = ['purchase', 'records', 'checkout', 'stats'];  var idx = tabNames.indexOf(tab);  document.querySelectorAll('.tab')[idx].classList.add('ac');  document.getElementById('tc-' + tab).classList.add('ac');}
// ============================================================
// PURCHASE RECORDS
// ============================================================
var PURCHASE_KEY = 'csm_purchase_v1';var purchaseRecs = [];// 供应商采购记录var SUPPLIER_RECS_KEY = 'csm_supplier_recs_v1';var supplierRecs = [];var supplierRef = null;function loadPurchase() {  try {    var stored = localStorage.getItem(PURCHASE_KEY);    purchaseRecs = stored ? JSON.parse(stored) : [];  } catch(e) { purchaseRecs = []; }}function savePurchase() {  // Firebase 自动同步}function savePurchaseItem(item) {  if (purchaseRef && item.id) {    purchaseRef.child(item.id).set(item);  }}function openPurchaseForm() {  // 重置入库按钮状态  var checkInBtn = gid('checkInBtn');  if (checkInBtn) {    checkInBtn.classList.remove('btn-g');    checkInBtn.classList.add('btn-s');    checkInBtn.innerHTML = '✓ 入库 Check In';    checkInBtn.disabled = false;  }  // 重置表单  var cnField = gid('fp-cn');  var supplierField = gid('fp-supplier');  var dateField = gid('fp-date');  var itemsBody = gid('purchaseItemsBody');  purchaseItemRowCounter = 0;  if (cnField) cnField.value = '';  if (supplierField) supplierField.value = '';  if (dateField) dateField.value = nowFmt();  if (itemsBody) {    itemsBody.innerHTML = '<tr class="purchase-item-row">' +      '<td style="padding:4px;border:1px solid #ddd;position:relative">' +        '<input type="text" class="item-product" placeholder="品名" data-rowid="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px" oninput="showSuggestPurchaseItem(this)" onfocus="showSuggestPurchaseItem(this)" onblur="setTimeout(function(){hideSuggest(\'purchase-item-0\')},200)">' +        '<div class="suggest-list" id="suggest-purchase-item-0"></div>' +      '</td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-qty" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-demurrage" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-customs" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-coldfee" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-attestation" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-repack" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd;text-align:center"><button type="button" class="abtn x" onclick="removePurchaseItem(this)" style="color:#ff4444;font-size:16px">×</button></td>' +      '</tr>';  }  gid('purchaseModal').classList.add('sh');}function clPurchaseModal() {  // 重置表单  var cnField = gid('fp-cn');  var supplierField = gid('fp-supplier');  var dateField = gid('fp-date');  var itemsBody = gid('purchaseItemsBody');  purchaseItemRowCounter = 0;  if (cnField) cnField.value = '';  if (supplierField) supplierField.value = '';  if (dateField) dateField.value = '';  if (itemsBody) {    itemsBody.innerHTML = '<tr class="purchase-item-row">' +      '<td style="padding:4px;border:1px solid #ddd;position:relative">' +        '<input type="text" class="item-product" placeholder="品名" data-rowid="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px" oninput="showSuggestPurchaseItem(this)" onfocus="showSuggestPurchaseItem(this)" onblur="setTimeout(function(){hideSuggest(\'purchase-item-0\')},200)">' +        '<div class="suggest-list" id="suggest-purchase-item-0"></div>' +      '</td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-qty" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-demurrage" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-customs" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-coldfee" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-attestation" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-repack" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +      '<td style="padding:4px;border:1px solid #ddd;text-align:center"><button type="button" class="abtn x" onclick="removePurchaseItem(this)" style="color:#ff4444;font-size:16px">×</button></td>' +      '</tr>';  }  gid('purchaseModal').classList.remove('sh');}function addPurchase() {  console.log('addPurchase called');  var cn = (gid('fp-cn').value || '').trim().toUpperCase();  var supplier = (gid('fp-supplier').value || '').trim();  var purchaseDate = gid('fp-date').value;  if (!cn) { toast('请输入集装箱号 / Enter container no.', 'err'); return; }  if (!supplier) { toast('请输入供应商', 'err'); return; }  // 获取所有品名行  var rows = document.querySelectorAll('.purchase-item-row');  console.log('Found rows:', rows.length);  var items = [];  rows.forEach(function(row, idx) {    var productInput = row.querySelector('.item-product');    var product = (productInput.value || '').trim();    console.log('Row', idx, 'product:', product);    if (!product) return; // 跳过空品名行    var item = {      qty: parseFloat(row.querySelector('.item-qty').value) || 0,      demurrage: parseFloat(row.querySelector('.item-demurrage').value) || 0,      customs: parseFloat(row.querySelector('.item-customs').value) || 0,      coldFee: parseFloat(row.querySelector('.item-coldfee').value) || 0,      attestation: parseFloat(row.querySelector('.item-attestation').value) || 0,      repack: parseFloat(row.querySelector('.item-repack').value) || 0    };    // 为每个品名创建单独的采购记录    var id = Date.now().toString(36) + Math.random().toString(36).substr(2, 5) + idx;    var rec = {      id: id,      cn: cn,      supplier: supplier,      product: product,      purchaseDate: purchaseDate,      qty: item.qty,      demurrage: item.demurrage,      customs: item.customs,      coldFee: item.coldFee,      attestation: item.attestation,      repack: item.repack,      waste: 0,      other: 0    };    items.push(rec);    console.log('Added item:', rec.product);    // 生成序列号（使用采购日期）并保存到 Firebase    generateSeq(function(seq) {      rec.seq = seq;      if (purchaseRef) {        purchaseRef.child(id).set(rec).then(function(){console.log('Saved:',id,'seq:',seq);}).catch(function(e){console.error('Error:',e);});      }    }, purchaseDate ? new Date(purchaseDate + 'T00:00:00') : null);  });  console.log('Total items to add:', items.length);  if (items.length === 0) {    toast('请至少添加一个品名', 'err');    return;  }  // 注意：数据会通过 Firebase 监听器自动添加到 purchaseRecs，这里不需要手动 concat  // 只需要保存到 Firebase 即可  // 重置表单  gid('fp-cn').value = '';  gid('fp-supplier').value = '';  gid('fp-date').value = '';  gid('purchaseItemsBody').innerHTML = '<tr class="purchase-item-row">' +    '<td style="padding:4px;border:1px solid #ddd"><input type="text" class="item-product" placeholder="品名" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-qty" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-demurrage" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-customs" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-coldfee" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-attestation" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-repack" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd;text-align:center"><button type="button" class="abtn x" onclick="removePurchaseItem(this)" style="color:#ff4444;font-size:16px">×</button></td>' +    '</tr>';  clPurchaseModal();  renderPurchase();  toast('✅ 已添加 ' + items.length + ' 条采购记录', 'ok');}// 添加品名行function addPurchaseItem() {  purchaseItemRowCounter++;  var rowId = purchaseItemRowCounter;  var newRow = document.createElement('tr');  newRow.className = 'purchase-item-row';  newRow.innerHTML =    '<td style="padding:4px;border:1px solid #ddd;position:relative">' +      '<input type="text" class="item-product" placeholder="品名" data-rowid="' + rowId + '" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px" oninput="showSuggestPurchaseItem(this)" onfocus="showSuggestPurchaseItem(this)" onblur="setTimeout(function(){hideSuggest(\'purchase-item-' + rowId + '\')},200)">' +      '<div class="suggest-list" id="suggest-purchase-item-' + rowId + '"></div>' +    '</td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-qty" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-demurrage" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-customs" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-coldfee" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-attestation" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd"><input type="number" class="item-repack" value="0" min="0" style="width:100%;padding:5px;border:1px solid #ddd;border-radius:3px;text-align:center"></td>' +    '<td style="padding:4px;border:1px solid #ddd;text-align:center"><button type="button" class="abtn x" onclick="removePurchaseItem(this)" style="color:#ff4444;font-size:16px">×</button></td>';  document.getElementById('purchaseItemsBody').appendChild(newRow);}// 删除品名行function removePurchaseItem(btn) {  var rows = document.querySelectorAll('.purchase-item-row');  if (rows.length > 1) {    btn.closest('tr').remove();  } else {    toast('至少保留一行品名', 'err');  }}function delPurchase(id) {  if (!confirm('确认删除这条采购记录？ / Confirm delete?')) return;  if (purchaseRef) {    purchaseRef.child(id).remove();  }}function filterPurchase() { renderPurchase(); }function resetPurchaseSearch() {  gid('search-purchase-date').value = '';  gid('search-purchase-cn').value = '';  gid('search-purchase-supplier').value = '';  renderPurchase();}function renderPurchase() { console.log("purchaseRecs:", purchaseRecs);  var tb = gid('tb-purchase');  var es = gid('es-purchase');  if (!tb || !es) return;  // 获取搜索条件  var searchDate = (gid('search-purchase-date').value || '').trim();  var searchCn = (gid('search-purchase-cn').value || '').trim().toUpperCase();  var searchSupplier = (gid('search-purchase-supplier').value || '').trim().toUpperCase();  // 过滤 warehouse1 采购记录  var filteredRecs = purchaseRecs.filter(function(r) {    var matchDate = !searchDate || (r.purchaseDate || '').indexOf(searchDate) !== -1;    var matchCn = !searchCn || (r.cn || '').indexOf(searchCn) !== -1;    var matchSupplier = !searchSupplier || (r.supplier || '').toUpperCase().indexOf(searchSupplier) !== -1;    return matchDate && matchCn && matchSupplier;  });  // 合并供应商记录：只显示尚未在 warehouse1 采购列表中的记录  var warehouseCNs = {};  purchaseRecs.forEach(function(r) { if (r.cn) warehouseCNs[r.cn] = true; });  var supplierOnlyRecs = supplierRecs.filter(function(r) {    if (!r.cn) return false;    if (warehouseCNs[r.cn]) return false; // 已在 warehouse1 中，不再重复显示    var matchDate = !searchDate || (r.purchaseDate || '').indexOf(searchDate) !== -1;    var matchCn = !searchCn || (r.cn || '').indexOf(searchCn) !== -1;    var matchSupplier = !searchSupplier || (r.supplier || '').toUpperCase().indexOf(searchSupplier) !== -1;    return matchDate && matchCn && matchSupplier;  });  if (filteredRecs.length === 0 && supplierOnlyRecs.length === 0) { tb.innerHTML = ''; es.style.display = 'block'; return; }  es.style.display = 'none';  // 按集装箱号分组  var cnGroups = {};  filteredRecs.forEach(function(r) {    var key = r.cn || '_empty_';    if (!cnGroups[key]) cnGroups[key] = [];    cnGroups[key].push(r);  });  var html = '';  Object.keys(cnGroups).sort().forEach(function(cn) {    var items = cnGroups[cn];    var groupId = 'group-' + cn.replace(/[^a-zA-Z0-9]/g, '');    var firstItem = items[0];    var totalItems = items.length;    var totalAmount = items.reduce(function(s, r) { return s + ((r.demurrage||0)+(r.customs||0)+(r.coldFee||0)+(r.attestation||0)+(r.repack||0)+(r.waste||0)+(r.other||0)); }, 0);    var purchaseDate = firstItem.purchaseDate ? fdt(firstItem.purchaseDate+'T00:00:00') : '-';    // 检查是否有入库记录    var hasInRec = false;    var coldFeeDisplay = '-';    for (var i = 0; i < recs.length; i++) {      if (recs[i].cn === cn && !recs[i].type && recs[i].store === currentColdStore) {        hasInRec = true;        var actualFee = calcActualFee(recs[i]);        if (actualFee > 0) {          coldFeeDisplay = '<strong style="color:#ff9900;background:#fff8e1;padding:2px 6px;border-radius:3px;font-size:14px">' + actualFee.toFixed(2) + '</strong>';        } else {          coldFeeDisplay = '<span style="color:#666;background:#fff8e1;padding:2px 6px;border-radius:3px;font-size:14px">0.00</span>';        }        break;      }    }    if (cn === '_empty_') cn = '-';    // 主行：集装箱号 + 展开按钮    var expandBtn = totalItems > 1 ?      '<button type="button" class="abtn" style="background:#f0f0f0;border:1px solid #ddd;padding:2px 6px;font-size:14px" onclick="togglePurchaseGroup(\'' + groupId + '\',this)">+</button>' : '';    var firstProduct = firstItem.product || '-';    var firstQty = firstItem.qty || '-';    var firstSeq = firstItem.seq || '-';    html += '<tr style="background:#f9f9f9;font-weight:bold" id="pur-main-' + groupId + '">' +      '<td style="font-size:13px;color:#0066cc">' + firstSeq + '</td>' +      '<td>' + expandBtn + ' <button type="button" class="abtn" style="background:#e8f4ff;border-color:#00bfff;color:#00bfff;padding:2px 6px;font-size:11px" onclick="quickCheckIn(\'' + firstItem.id + '\')">📥</button> ' + cn + ' <span style="color:#999;font-size:11px">(' + totalItems + '品名)</span></td>' +      '<td style="font-family:Arial;text-transform:capitalize">'+(firstItem.supplier||'-')+'</td><td style="font-family:Arial">'+firstProduct+'</td><td>'+purchaseDate+'</td><td style="font-family:Arial">'+firstQty+'</td><td style="font-family:Arial">'+(firstItem.demurrage||0)+'</td><td style="font-family:Arial">'+(firstItem.customs||0)+'</td><td style="font-family:Arial">'+(coldFeeDisplay||'-')+'</td><td style="font-family:Arial">'+(firstItem.attestation||0)+'</td><td style="font-family:Arial">'+(firstItem.repack||0)+'</td><td style="font-family:Arial">'+(firstItem.waste||0)+'</td><td style="font-family:Arial">'+(firstItem.other||0)+'</td>' +      '<td><strong style="color:#0066cc">'+totalAmount.toFixed(2)+'</strong></td>' +      '<td><button type="button" class="abtn" onclick="openEditPurchase(\''+firstItem.id+'\')">✏️</button><button type="button" class="abtn x" onclick="delPurchaseGroup(\'' + cn + '\')">🗑</button></td></tr>';    // 子行：每个品名    items.forEach(function(r) {      var total = (r.demurrage||0)+(r.customs||0)+(r.coldFee||0)+(r.attestation||0)+(r.repack||0)+(r.waste||0)+(r.other||0);      html += '<tr class="purchase-sub-row ' + groupId + '" style="display:none;background:#fff">' +        '<td style="font-size:13px;color:#0066cc">' + (r.seq || '-') + '</td>' +        '<td style="padding-left:40px;color:#666">└ '+(r.product||'-')+'</td>' +        '<td style="font-family:Arial;text-transform:capitalize;color:#666">'+(r.supplier||'-')+'</td><td style="font-family:Arial;text-transform:capitalize">'+(r.product||'-')+'</td><td>-</td><td>'+(r.qty||0)+'</td>' +        '<td>'+(r.demurrage||0)+'</td><td>'+(r.customs||0)+'</td><td>-</td>' +        '<td>'+(r.attestation||0)+'</td><td>'+(r.repack||0)+'</td><td>'+(r.waste||0)+'</td><td>'+(r.other||0)+'</td>' +        '<td><strong style="color:#0066cc">'+total.toFixed(2)+'</strong></td>' +        '<td><button type="button" class="abtn" onclick="openEditPurchase(\''+r.id+'\')">✏️</button><button type="button" class="abtn x" onclick="delPurchase(\''+r.id+'\')">🗑</button></td></tr>';    });  });  // 显示供应商专属记录（在 warehouse1 采购列表中没有的）  if (supplierOnlyRecs.length > 0) {    // 按采购日期排序    supplierOnlyRecs.sort(function(a, b) { return new Date(b.purchaseDate) - new Date(a.purchaseDate); });    supplierOnlyRecs.forEach(function(r) {      var supplierLabel = '<span style="background:#fff3e0;color:#e65100;font-size:10px;padding:1px 4px;border-radius:2px;margin-left:4px">供应商</span>';      var cnClickFn = "openSupplierCNDetailByCN('" + r.cn + "')";      html += '<tr style="background:#fffbf5">' +        '<td style="font-size:13px;color:#ff9900">' + (r.seq || '-') + '</td>' +        '<td><a href="javascript:void(0)" onclick="' + cnClickFn + '" style="color:#ff9900;font-weight:bold;text-decoration:underline">' + (r.cn || '-') + '</a>' + supplierLabel + '</td>' +        '<td style="font-family:Arial;text-transform:capitalize;color:#666">' + (r.supplier || '-') + '</td>' +        '<td style="font-family:Arial;text-transform:capitalize">' + (r.product || '-') + '</td>' +        '<td>' + (r.purchaseDate ? fdt(r.purchaseDate+'T00:00:00') : '-') + '</td>' +        '<td style="font-family:Arial">' + (r.qty || 0) + '</td>' +        '<td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td>' +        '<td style="color:#999">待入库</td>' +        '<td><button type="button" class="abtn" onclick="editSupplierRecByCN(\'' + r.cn + '\')">✏️</button></td>' +      '</tr>';    });  }  tb.innerHTML = html;}// 按集装箱号查找供应商记录并弹出详情function openSupplierCNDetailByCN(cn) {  var rec = supplierRecs.find(function(r) { return r.cn === cn; });  if (!rec) return;  openSupplierCNDetail(rec.id);}// 按集装箱号编辑供应商记录function editSupplierRecByCN(cn) {  var rec = supplierRecs.find(function(r) { return r.cn === cn; });  if (!rec) return;  editSupplierRec(rec.id);}// 展开/折叠采购组function togglePurchaseGroup(groupId, btn) { console.log('togglePurchaseGroup:', groupId);  var rows = document.querySelectorAll('.purchase-sub-row.' + groupId);  var isExpanded = btn.textContent === '-';  rows.forEach(function(row) {    row.style.display = isExpanded ? 'none' : '';  });  btn.textContent = isExpanded ? '+' : '-';}// 删除整组采购记录function delPurchaseGroup(cn) {  if (!confirm('确认删除集装箱 ' + cn + ' 的所有采购记录？')) return;  var toDelete = purchaseRecs.filter(function(r) { return r.cn === cn; });  toDelete.forEach(function(r) {    if (purchaseRef) purchaseRef.child(r.id).remove();  });}
// ============================================================
