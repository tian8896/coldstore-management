...
// 用户未登录，显示登录弹窗        
console.log('Firebase Auth: User not logged in');        currentUser = null;        currentUserEmail = null;        isAdmin = false;        isStaff = false;        isLogistics = false;        isSupplier = false;        currentSupplierName = null;        currentMainSuite = 'w1';        try { window.isAdmin = false; window.isStaff = false; } catch (eW2) {}        try { sessionStorage.removeItem('csm_main_suite'); } catch (eAu) {}        var shellAu = gid('adminPortalShell');        if (shellAu) { shellAu.style.display = 'none'; shellAu.setAttribute('aria-hidden', 'true'); }        if (typeof resetMainSuiteForNonAdmin === 'function') resetMainSuiteForNonAdmin();        var h1Au = gid('headerTitle');        var hpAu = gid('headerSubtitle');        if (h1Au) h1Au.textContent = '🧊 迪拜大丰收冷库管理系统';        if (hpAu) hpAu.textContent = 'Super Harvest Cold Store Management System - Warehouse 1';        var ls = document.querySelector('.login-screen');        if (ls) ls.classList.remove('hidden');        showLoginModal();      }    });    
...
// 退出登录
function doLogout() {  auth.signOut()    .then(function() {      console.log('Firebase Auth: Logged out');      currentUser = null;      currentUserEmail = null;      isAdmin = false;      isStaff = false;      isLogistics = false;      isSupplier = false;      currentSupplierName = null;      currentMainSuite = 'w1';      try { window.isAdmin = false; window.isStaff = false; } catch (eW3) {}      try { sessionStorage.removeItem('csm_main_suite'); } catch (eLo) {}      
// 重置界面      
...
function openSupplierCNDetail(id) {
  ...
  html += supplierItems.map(function(item) {
    return '<tr><td style="padding:6px;border:1px solid #ddd;font-family:Arial;text-transform:capitalize">' + w1ProductHtml(item.product) + '</td><td style="padding:6px;border:1px solid #ddd;text-align:center">' + (item.qty || 0) + '</td></tr>';
  }).join('');
  html += '</table></div>';
  if (linked.length) {
    ...
    html += linked.map(function(item) {
      ...
      return '<tr>' +
        '<td style="padding:4px;border:1px solid #c8e6c9;font-family:Arial;text-transform:capitalize">' + w1ProductHtml(item.product) + '</td>' +
  ...
  if (breakdown.rows.length) {
    ...
    breakdown.rows.forEach(function(row) {
      html += '<tr>' +
        ...
        '<td style="padding:6px;border:1px solid #ddd;font-family:Arial;text-transform:capitalize">' + w1ProductHtml(row.product) + '</td>' +
...
// 管理员才显示修改按钮        
var editOutBtn = (isAdmin || isStaff) ? '<button class="abtn" onclick="showEditOutRecord(\'' + or.id + '\')" style="margin-left:4px">✏️</button>' : '';        html.push('<tr style="background:#f0fff0">' +          '<td style="padding-left:20px;color:#0066cc;font-size:13px">' + (or.seq || '-') + '</td>' +          '<td style="padding-left:20px;color:#999">' + cn + '</td>' +          '<td style="color:#999;font-family:Arial">' + fmtSupplierName(or.supplier) + '</td>' +          '<td style="color:#999;font-family:Arial;text-transform:capitalize">' + w1ProductHtml(or.product) + '</td>' +          '<td style="color:#00aa00;font-weight:bold">' + fdt(or.dep) + '</td>' +          '<td>-</td>' +          '<td><span style="color:#cc0000;font-weight:bold">' + or.pallets_out + '</span> / <strong style="color:#00aa00">' + (outRemPallets >= 0 ? outRemPallets : '0') + '</strong></td>' +          '<td><span style="color:#cc0000;font-weight:bold">' + or.items_out + '</span></td>' +          '<td><strong style="color:#00aa00">' + (outRemItems >= 0 ? outRemItems : '0') + '</strong></td>' +          '<td>-</td>' +          '<td>-</td>' +          '<td>-</td>' +          '<td>-</td>' +          '<td>' + editOutBtn + '</td>' +          '</tr>');      });      currentDate = new Date(weekEnd);      currentDate.setDate(currentDate.getDate() + 1);      weekNum++;    }  });  tb.innerHTML = html.join('');}
// ============================================================
// 搜索和导出功能
// ============================================================
